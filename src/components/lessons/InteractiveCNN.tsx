...
export function InteractiveCNN() {
  const architecture = [
    ...
    { name: "Conv2D + ReLU", type: "op", desc: "Extracts local features (edges)", color: "text-indigo-700" },
    { name: "Conv Block 1", type: "data", shape: "32×32×16", desc: "Feature Maps", w: 32, h: 32, d: 16, color: "bg-indigo-500" },
    { name: "MaxPool (2×2)", type: "op", desc: "Downsamples spatially", color: "text-amber-600" },
    { name: "Pool 1", type: "data", shape: "16×16×16", desc: "Compressed Features", w: 16, h: 16, d: 16, color: "bg-indigo-600" },
    ...
    { name: "Flatten", type: "op", desc: "Unrolls into 1D vector", color: "text-rose-600" },
    { name: "1D Vector", type: "data", shape: "2048", desc: "Raw Features", w: 4, h: 4, d: 100, color: "bg-gray-500", is1D: true },
    { name: "Dense (128)", type: "op", desc: "Learns combinations", color: "text-emerald-600" },
    { name: "Hidden Layer", type: "data", shape: "128", desc: "Activations", w: 4, h: 4, d: 64, color: "bg-emerald-500", is1D: true },
    { name: "Dense (10)", type: "op", desc: "Computes logits", color: "text-emerald-600" },
    { name: "Output Probabilities", type: "data", shape: "10", desc: "Class Predictions", w: 4, h: 4, d: 10, color: "bg-rose-500", is1D: true }
  ];
  ...
  return (
    <div className="my-8 flex flex-col gap-6 p-6 bg-white border border-rose-200 rounded-xl shadow-xl overflow-hidden">
      <div className="flex justify-between items-center -mt-2">
        <h2 className="text-xl font-bold text-rose-950 tracking-tight">CNN Architecture Pipeline (VGG-style)</h2>
      </div>

      <div className="bg-rose-50 rounded-xl border border-rose-200 p-6 overflow-x-auto custom-scrollbar">
          ...
          {architecture.map((layer, i) => {
            if (layer.type === "op") {
              return (
                <div key={i} className="flex flex-col items-center justify-center w-32 px-2 shrink-0">
                  ...
                  <div className="text-[10px] text-rose-700 text-center uppercase tracking-wider mb-2 h-6">{layer.desc}</div>
                  <div className="h-0.5 w-full bg-gradient-to-r from-transparent via-gray-700 to-transparent relative">
                    <motion.div 
                      className="absolute inset-y-0 left-0 bg-white w-4"
            ...
            return (
              <div key={i} className="flex flex-col items-center justify-end h-[300px] shrink-0 mx-2">
                  ...
                  {!layer.is1D ? (
                    ...
                    >
                      ...
                      {Array.from({ length: Math.min(10, Math.ceil(pxH / 6)) }).map((_, j) => (
                        <div key={j} className="h-px bg-white/20 w-full mt-[5px]" />
                      ))}
                    </motion.div>
                  )}
                </div>
                ...
                {/* Labels */}
                <div className="bg-rose-100 border border-rose-200 rounded p-2 text-center w-full min-w-[100px] shadow-lg">
                  <div className="font-bold text-rose-950 text-xs mb-1">{layer.name}</div>
                  <div className="font-mono text-cyan-600 text-[10px] bg-rose-50 py-0.5 rounded border border-cyan-900/30">
                    {layer.shape}
                  </div>
                </div>
              </div>
      ...
      <p className="text-xs text-rose-700 leading-relaxed text-center px-4">
