  ...
  return (
    <div className="my-8 flex flex-col gap-6 p-6 bg-white border border-rose-200 rounded-xl shadow-xl">
      <div className="flex justify-between items-center -mt-2">
        <h2 className="text-xl font-bold text-rose-950 tracking-tight">The U-Net Architecture</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          <div className="p-4 bg-rose-100 rounded-lg border border-rose-200 min-h-[250px]">
            {hoverState === "none" && (
              <div className="flex flex-col gap-4">
                <h3 className="text-sm font-medium text-cyan-600">Hover over the diagram</h3>
            ...
            {hoverState === "down" && (
              <div className="flex flex-col gap-4">
                <h3 className="text-sm font-medium text-indigo-700">Downsampling (Encoder)</h3>
                <p className="text-sm text-rose-600 leading-relaxed">
            ...
            {hoverState === "up" && (
              <div className="flex flex-col gap-4">
                <h3 className="text-sm font-medium text-rose-600">Upsampling (Decoder)</h3>
                <p className="text-sm text-rose-600 leading-relaxed">
            ...
            {hoverState === "skip" && (
              <div className="flex flex-col gap-4">
                <h3 className="text-sm font-medium text-emerald-600">Skip Connections</h3>
                <p className="text-sm text-rose-600 leading-relaxed">
        ...
        <div className="lg:col-span-8 flex flex-col items-center justify-center bg-rose-50 rounded-xl border border-rose-200 p-8 min-h-[400px]">
          ...
          <div className="relative w-full max-w-lg h-80 flex justify-between items-start">
            ...
            <div className="absolute left-0 top-0 -translate-x-1/2 -translate-y-8 flex flex-col items-center">
              <span className="text-[10px] font-mono text-rose-700 mb-1">Input $x_t$</span>
              <div className="w-12 h-12 bg-black border border-rose-300 rounded shadow-lg overflow-hidden flex items-center justify-center">
            ...
            <div className="absolute right-0 top-0 translate-x-1/2 -translate-y-8 flex flex-col items-center">
              <span className="text-[10px] font-mono text-rose-700 mb-1 font-bold text-center">Noise Prediction<br/>$\epsilon_\theta$</span>
              <div className="w-12 h-12 bg-black border border-rose-900 rounded shadow-lg overflow-hidden flex items-center justify-center">
            ...
            >
              ...
              <div className={`mt-2 text-[10px] font-bold tracking-widest uppercase transition-colors ${hoverState === "down" ? 'text-indigo-700' : 'text-rose-600'}`}>
                Downsample
              </div>
            </div>
            ...
            >
              ...
              />
              <div className={`mt-2 text-[10px] font-bold tracking-widest uppercase transition-colors ${hoverState === "bottle" ? 'text-fuchsia-600' : 'text-rose-600'}`}>
                Bottleneck
              </div>
            </div>
            ...
            >
              {[64, 48, 32].reverse().map((size, i) => (
                <motion.div 
                  key={`up-${i}`}
                  className={`border-2 rounded bg-rose-950/40 flex items-center justify-center transition-colors ${hoverState === "up" ? 'border-rose-400 shadow-[0_0_15px_rgba(251,113,133,0.5)]' : 'border-rose-900/50'}`}
                  style={{ width: size, height: size }}
                  animate={{ scale: hoverState === "up" ? 1.05 : 1 }}
                />
              ))}
              <div className={`mt-2 text-[10px] font-bold tracking-widest uppercase transition-colors ${hoverState === "up" ? 'text-rose-600' : 'text-rose-600'}`}>
                Upsample
              </div>
            </div>
