  ...
  return (
    <div className="my-8 flex flex-col gap-6 p-6 bg-white border border-rose-200 rounded-xl shadow-xl">
      <div className="flex justify-between items-center -mt-2">
        <h2 className="text-xl font-bold text-rose-950 tracking-tight">Bounding Box Regression</h2>
        <div className={`text-sm font-bold px-3 py-1 rounded-full border ${iou > 0.7 ? 'bg-emerald-100 text-emerald-600 border-emerald-500/50' : 'bg-red-900/50 text-red-400 border-red-500/50'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="flex flex-col gap-6">
          <div className="p-4 bg-rose-100 rounded-lg border border-rose-200 flex flex-col gap-4">
            <h3 className="text-sm font-medium text-cyan-600 mb-2">Network Outputs (Regression Head)</h3>
            ...
            <div className="bg-rose-50 mt-2 p-3 rounded border border-rose-200 font-mono text-xs text-rose-600">
              <span className="text-rose-700 block mb-1">Final Vector Output:</span>
              [Class: Car (98%), <span className="text-cyan-600">X:{px}</span>, <span className="text-cyan-600">Y:{py}</span>, <span className="text-amber-600">W:{pw}</span>, <span className="text-amber-600">H:{ph}</span>]
            </div>
          </div>
        </div>

        {/* Visualizer */}
        <div className="flex flex-col items-center justify-center bg-rose-50 rounded-xl border border-rose-200 p-6">
          <div 
            className="relative bg-rose-100 border border-rose-300 overflow-hidden shadow-2xl"
          ...
          >
            ...
            >
              <span className={`absolute -top-5 left-0 text-[10px] font-mono font-bold ${iou > 0.7 ? 'text-emerald-600' : 'text-cyan-600'}`}>Prediction</span>
            </div>
          </div>

          <p className="text-xs text-rose-700 mt-6 max-w-[90%] text-center leading-relaxed">
