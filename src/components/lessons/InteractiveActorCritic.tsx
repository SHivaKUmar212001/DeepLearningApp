  ...
  return (
    <div className="my-8 flex flex-col gap-6 p-6 bg-white border border-rose-200 rounded-xl shadow-xl">
      <div className="flex justify-between items-center -mt-2">
        <h2 className="text-xl font-bold text-rose-950 tracking-tight">Actor-Critic Architecture</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          <div className="p-4 bg-rose-100 rounded-lg border border-rose-200 flex flex-col gap-4">
            ...
            <div className="text-sm font-medium text-cyan-600">Actor: Choose a Move</div>
            
            <div className="flex flex-col gap-2">
              <button 
                onClick={() => handleAction("knight")}
                disabled={isUpdating}
                className="py-2 bg-indigo-100 hover:bg-indigo-600 disabled:opacity-50 border border-indigo-500/50 text-indigo-600 hover:text-rose-950 font-bold rounded transition-colors"
              >
                Move Knight ♞
              </button>
              <button 
        ...
        {/* Visualizer */}
        <div className="lg:col-span-8 flex flex-col items-center justify-center bg-rose-50 rounded-xl border border-rose-200 p-8 min-h-[350px]">
          ...
          <div className="relative w-full max-w-lg flex flex-col gap-12">
            ...
            <div className="flex justify-between items-center w-full">
              ...
              <div className="flex flex-col items-center gap-2 flex-1">
                <div className="text-[10px] font-bold text-rose-700 uppercase tracking-widest">Critic Network (Value)</div>
                <div className="w-32 h-20 bg-rose-50 border-2 border-rose-500/50 rounded-lg flex flex-col items-center justify-center shadow-[0_0_15px_rgba(244,63,94,0.1)]">
                  <span className="text-rose-600 font-bold tracking-widest">Evaluate</span>
                  <AnimatePresence>
                    {criticScore !== null && (
                      <motion.div 
                        initial={{ scale: 0 }}
                        animate={{ scale: 1 }}
                        className={`text-sm font-mono font-bold ${criticScore > 0 ? 'text-emerald-600' : 'text-rose-700'}`}
                      >
            ...
            {/* Bottom Row: Actor */}
            <div className="flex flex-col items-center w-full pt-8 border-t border-rose-200 relative">
              
              {/* Feedback Arrow from Critic to Actor */}
              {criticScore !== null && (
                <motion.svg 
                  className="absolute right-1/4 top-0 -translate-y-full w-8 h-24 text-rose-700 z-10"
              ...
              <div className="text-[10px] font-bold text-indigo-500 uppercase tracking-widest mb-2">Actor Network (Policy)</div>
              
              <div className="w-full flex justify-around">
                <div className="flex flex-col items-center gap-2">
                  <div className="h-24 w-4 bg-rose-100 rounded-full flex items-end overflow-hidden border border-rose-200">
                    <motion.div className="w-full bg-indigo-500" animate={{ height: `${probs.knight}%` }} />
                  </div>
                  <span className="text-[10px] font-mono text-rose-700">Knight</span>
                  <span className="text-xs font-bold text-indigo-700">{probs.knight.toFixed(1)}%</span>
                </div>

                <div className="flex flex-col items-center gap-2">
                  <div className="h-24 w-4 bg-rose-100 rounded-full flex items-end overflow-hidden border border-rose-200">
                    <motion.div className="w-full bg-indigo-500" animate={{ height: `${probs.pawn}%` }} />
                  </div>
                  <span className="text-[10px] font-mono text-rose-700">Pawn</span>
                  <span className="text-xs font-bold text-indigo-700">{probs.pawn.toFixed(1)}%</span>
                </div>

                <div className="flex flex-col items-center gap-2">
                  <div className="h-24 w-4 bg-rose-100 rounded-full flex items-end overflow-hidden border border-rose-200">
                    <motion.div className="w-full bg-indigo-500" animate={{ height: `${probs.queen}%` }} />
                  </div>
                  <span className="text-[10px] font-mono text-rose-700">Queen</span>
                  <span className="text-xs font-bold text-indigo-700">{probs.queen.toFixed(1)}%</span>
                </div>
              </div>
