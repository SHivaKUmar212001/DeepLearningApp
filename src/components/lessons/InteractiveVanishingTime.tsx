  ...
  return (
    <div className="my-8 flex flex-col gap-6 p-6 bg-white border border-rose-200 rounded-xl shadow-xl">
      <div className="flex justify-between items-center -mt-2">
        <h2 className="text-xl font-bold text-rose-950 tracking-tight">Vanishing Gradients in Time</h2>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="flex flex-col gap-6">
          <div className="p-4 bg-rose-100 rounded-lg border border-rose-200 flex flex-col gap-4">
            <h3 className="text-sm font-medium text-cyan-600">Recurrent Weight ({"$W_{hh}$"})</h3>
        ...
        <div className="lg:col-span-2 flex flex-col items-center justify-center bg-rose-50 rounded-xl border border-rose-200 p-6 overflow-hidden">
          ...
          <div className="w-full max-w-2xl">
            <h3 className="text-xs font-mono text-rose-700 uppercase tracking-wider font-bold mb-6 text-center">
              Gradient Signal Reaching Time $t$
            </h3>

            <div className="flex items-end justify-between h-48 border-b border-rose-300 relative w-full px-2">
              ...
              {sentence.map((word, i) => {
                ...
                else if (grad < 1) barColor = "bg-rose-300";
                ...
                return (
                  <div key={i} className="flex flex-col items-center relative w-full group">
                    ...
                    {/* Value Tooltip */}
                    <div className="absolute -top-8 opacity-0 group-hover:opacity-100 transition-opacity bg-rose-100 border border-rose-300 text-xs font-mono px-2 py-1 rounded text-rose-950 z-10 pointer-events-none">
                    ...
                    <div className="absolute top-full mt-2 w-full flex justify-center">
                      <span className={`text-[10px] font-mono whitespace-nowrap origin-top-left -rotate-45 translate-y-2
                        ${isSource ? 'text-indigo-700 font-bold' : isTarget ? 'text-emerald-600 font-bold' : 'text-rose-700'}`}
                      >
                        {word}
                      </span>
                    </div>
                    ...
                    {/* Annotations */}
                    {isSource && (
                      <div className="absolute top-full mt-12 text-[10px] font-bold text-indigo-700 whitespace-nowrap hidden md:block">
                        Long-Term<br/>Dependency
                      </div>
                    )}
                    {isTarget && (
                      <div className="absolute top-full mt-12 text-[10px] font-bold text-emerald-600 whitespace-nowrap hidden md:block text-right">
            ...
            <div className="mt-20 text-center space-y-2">
              <p className="text-sm font-medium text-rose-800">
                Signal at &quot;France&quot;: <span className={`font-mono font-bold ${getGradient(sourceIndex) < 1 ? 'text-red-400' : 'text-indigo-700'}`}>
