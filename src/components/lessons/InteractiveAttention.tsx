            ...
            <div className="flex flex-col gap-3">
              {targetWords.map((word, i) => (
                <button 
                  ...
                  className={`p-3 rounded border text-left transition-colors flex justify-between items-center ${activeTarget === i ? 'bg-indigo-900/40 border-indigo-500/50 text-teal-400' : 'bg-[#111128] border-white/[0.08] text-white/40 hover:border-white/[0.1]'}`}
                >
                ...
                {sourceWords.map((word, i) => {
                  ...
                  return (
                    <div key={`src-${i}`} className="flex flex-col items-center relative z-10 w-20">
                      ...
                      <div className={`absolute -bottom-8 px-2 py-0.5 rounded-full text-[10px] font-mono font-bold transition-all duration-300 ${weight > 0.5 ? 'bg-teal-600 text-white/90' : 'bg-white/[0.06] text-white/40'}`}>
              ...
              <div className="flex justify-between w-full">
                {targetWords.map((word, i) => (
                  <div key={`tgt-${i}`} className="flex flex-col items-center relative z-10 w-20">
                    ...
                    <div className={`p-2 rounded-lg font-bold text-center w-full transition-all duration-300 ${activeTarget === i ? 'bg-[#0a0a18]/80 text-white/20 border-2 border-teal-600/40 scale-110 shadow-[0_0_15px_rgba(244,63,94,0.3)]' : 'bg-white/[0.04] text-white/40 border border-white/[0.08]'}`}>
                      {word}
                    </div>
                  </div>
                ))}
