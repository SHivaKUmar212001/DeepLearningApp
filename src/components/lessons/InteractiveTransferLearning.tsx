  ...
  return (
    <div className="my-8 flex flex-col gap-6 p-6 bg-white border border-rose-200 rounded-xl shadow-xl">
      <div className="flex justify-between items-center -mt-2">
        <h2 className="text-xl font-bold text-rose-950 tracking-tight">Transfer Learning Pipeline</h2>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          <div className="p-4 bg-rose-100 rounded-lg border border-rose-200 flex flex-col gap-4">
            <h3 className="text-sm font-medium text-cyan-600">Select Target Task</h3>
            
            <div className="flex flex-col gap-3">
              <button 
                onClick={() => setTask("imagenet")}
                className={`p-3 rounded border text-left transition-colors ${task === "imagenet" ? 'bg-indigo-900/40 border-indigo-500/50 text-indigo-600' : 'bg-rose-50 border-rose-200 text-rose-700 hover:border-rose-300'}`}
              >
              ...
              <button 
                ...
                className={`p-3 rounded border text-left transition-colors ${task === "custom" ? 'bg-emerald-900/40 border-emerald-500/50 text-emerald-300' : 'bg-rose-50 border-rose-200 text-rose-700 hover:border-rose-300'}`}
              >
            ...
            <div className="mt-2 bg-rose-50 p-3 rounded border border-rose-200 text-xs text-rose-600">
              {task === "imagenet" ? (
        ...
        {/* Visualizer */}
        <div className="lg:col-span-8 flex flex-col items-center justify-center bg-rose-50 rounded-xl border border-rose-200 p-6 md:p-12 overflow-hidden min-h-[300px]">
          
          <div className="flex items-center w-full max-w-[600px] gap-2">
            
            {/* Input Data */}
            <div className="flex flex-col items-center shrink-0 w-24">
              <div className="text-xs font-bold text-rose-700 mb-2">Input Image</div>
              <div className="w-16 h-16 rounded overflow-hidden border border-rose-300 flex items-center justify-center text-3xl bg-rose-200">
                {task === "imagenet" ? "🐶" : "🩻"}
              </div>
            </div>
            ...
            <div className="flex-1 flex flex-col items-center relative">
              <div className="text-xs font-bold text-rose-700 mb-2">Convolutional Base</div>
              <div className={`w-full h-24 rounded-lg border-2 flex items-center justify-center relative overflow-hidden transition-colors duration-500 ${task === "custom" ? "bg-blue-900/20 border-blue-500/30" : "bg-indigo-900/20 border-indigo-500/50"}`}>
                <span className={`font-mono text-sm font-bold z-10 ${task === "custom" ? "text-blue-400" : "text-indigo-700"}`}>ResNet-50</span>
              ...
              <div className={`mt-2 text-[10px] font-bold px-2 py-0.5 rounded ${task === "custom" ? "bg-blue-900/50 text-blue-300" : "bg-indigo-100 text-indigo-600"}`}>
                {task === "custom" ? "FROZEN WEIGHTS" : "TRAINABLE WEIGHTS"}
              </div>
            </div>

            <div className="text-rose-600 px-2">→</div>

            {/* Classification Head */}
            <div className="flex flex-col items-center shrink-0 w-32 relative">
              <div className="text-xs font-bold text-rose-700 mb-2">Dense Head</div>
              ...
              <div className={`w-full rounded-lg border-2 flex items-center justify-center relative overflow-hidden transition-all duration-500 bg-emerald-900/20 border-emerald-500/50 ${task === "imagenet" ? "h-32" : "h-16"}`}>
                <span className="font-mono text-xs font-bold text-emerald-600 z-10 relative">
